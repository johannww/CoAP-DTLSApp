...
def main():
    #cloud do loffi
    #RSA server key, server cert and ca cert
    #serverKeyAndCertFile = "serverkey.pem"
    #caCertFile = "ca_cert.pem"
    #ECDSA server key, server cert and ca cert
    serverKeyAndCertFile = "server_ecdsa.crt"
    caCertFile ="ca_ecdsa.crt"
    
    server = coapDtlsServer("0.0.0.0", 5684, serverKeyAndCertFile, caCertFile).server
    #server = coapDtlsServer("localhost", 5684, "serverkey.pem", "ca_cert.pem").server;
    priority = 0
    exit = False
    while not exit:
        try:
            cipher = SSL_get_cipher_list(server._socket._sock._sslobj._ssl.value, priority)
            print(cipher)
            priority += 1
        except:
            exit = True
